...
public class SystemIndividualFactor_editController
extends Controller {
    @FXML
    ...
    public ArrayList<NVO_BASELAND_INDIVIDUAL_FACTOR_STD> editData = null;
    public String defaultText = "";
    ArrayList<AnchorPane> nodeList = new ArrayList();
    NVO_BASELAND_INDIVIDUAL_FACTOR dataVo = null;
    IndividualFactorModel model = null;

    public void init(NVO_BASELAND_INDIVIDUAL_FACTOR nVO_BASELAND_INDIVIDUAL_FACTOR, IndividualFactorModel individualFactorModel, IBaseLandDialog iBaseLandDialog) {
        super.init(iBaseLandDialog);
        this.setTitle("\u500b\u5225\u56e0\u7d20\u57fa\u6e96\u8868-\u7b49\u7d1a\u6a19\u6e96\u8a2d\u5b9a");
        this.dataVo = nVO_BASELAND_INDIVIDUAL_FACTOR;
        this.model = individualFactorModel;
        individualFactorModel.editStd(nVO_BASELAND_INDIVIDUAL_FACTOR, EnumFactorType.INDIVIDUAL, this);
        this.initUI();
    ...
    private void initButton() {
        ...
        this.btOK.setOnAction((EventHandler)new EventHandler<ActionEvent>(){
            ...
            public void handle(ActionEvent actionEvent) {
                ArrayList<NVO_BASELAND_INDIVIDUAL_FACTOR_STD> arrayList = new ArrayList<NVO_BASELAND_INDIVIDUAL_FACTOR_STD>();
                arrayList.addAll(SystemIndividualFactor_editController.this.editData);
                if (SystemIndividualFactor_editController.this.numeral.isSelected()) {
                    SystemIndividualFactor_editController.this.saveTypeNumeral();
                } else if (SystemIndividualFactor_editController.this.selection.isSelected()) {
                    SystemIndividualFactor_editController.this.saveTypeSelection();
                } else if (SystemIndividualFactor_editController.this.custom.isSelected()) {
                    SystemIndividualFactor_editController.this.saveTypeCustom();
                } else {
                    JavaFXUtil.showErrorMessageBox("\u8acb\u9078\u64c7\u300c\u6a19\u6e96\u985e\u578b\u300d");
                    return;
                }
                SystemIndividualFactor_editController.this.model.saveStdData(SystemIndividualFactor_editController.this.dataVo, SystemIndividualFactor_editController.this.editData);
                JavaFXUtil.showToastMessageBox(new Stage(), "\u5132\u5b58\u6210\u529f", 1500);
                SystemIndividualFactor_editController.this.initUI();
            }
        });
    }
    ...
    private void initRadioButton() {
        this.numeral.selectedProperty().addListener((ChangeListener)new ChangeListener<Boolean>(){
            ...
            public void changed(ObservableValue<? extends Boolean> observableValue, Boolean bl, Boolean bl2) {
                if (bl2.booleanValue()) {
                    SystemIndividualFactor_editController.this.initUITypeNumeral();
                }
            }
        });
        this.selection.selectedProperty().addListener((ChangeListener)new ChangeListener<Boolean>(){
            ...
            public void changed(ObservableValue<? extends Boolean> observableValue, Boolean bl, Boolean bl2) {
                if (bl2.booleanValue()) {
                    SystemIndividualFactor_editController.this.initUITypeSelection();
                }
            }
        });
        this.custom.selectedProperty().addListener((ChangeListener)new ChangeListener<Boolean>(){
            ...
            public void changed(ObservableValue<? extends Boolean> observableValue, Boolean bl, Boolean bl2) {
                if (bl2.booleanValue()) {
                    SystemIndividualFactor_editController.this.initUITypeCustom();
                }
            }
        });
    }
    ...
    private void initUITypeCustom() {
        try {
            ...
            for (int i = 1; i <= this.dataVo.getDegree(); ++i) {
                ...
                NVO_BASELAND_INDIVIDUAL_FACTOR_STD nVO_BASELAND_INDIVIDUAL_FACTOR_STD = null;
                if (this.editData != null && this.editData.size() > 0 && this.editData.size() == stringArray.length) {
                    nVO_BASELAND_INDIVIDUAL_FACTOR_STD = this.editData.get(i - 1);
                }
                CustomPanel customPanel = new CustomPanel(anchorPane, nVO_BASELAND_INDIVIDUAL_FACTOR_STD, stringArray[i - 1], String.valueOf(i));
        ...
        try {
            ...
            for (int i = 1; i <= this.dataVo.getDegree(); ++i) {
                ...
                for (Node node : anchorPane.getChildren()) {
                    ...
                    if (nVO_BASELAND_INDIVIDUAL_FACTOR_STD == null) continue;
                    for (String string2 : stringArray2 = StringProcess.split(nVO_BASELAND_INDIVIDUAL_FACTOR_STD.getContents(), ":")) {
    ...
    private void initUITypeNumeral() {
        try {
            ...
            for (int i = 1; i <= this.dataVo.getDegree(); ++i) {
                ...
                LevelPanel levelPanel = new LevelPanel(anchorPane, nVO_BASELAND_INDIVIDUAL_FACTOR_STD, stringArray[i - 1], String.valueOf(i));
    ...
    private void saveTypeSelection() {
        ...
        for (int i = 1; i <= this.dataVo.getDegree(); ++i) {
            ...
            NVO_BASELAND_INDIVIDUAL_FACTOR_STD nVO_BASELAND_INDIVIDUAL_FACTOR_STD = this.getDefVo(i);
            for (Node node : anchorPane.getChildren()) {
                String string = node.getId();
                ...
                nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setContents(stringBuilder.toString());
            }
            this.editData.add(nVO_BASELAND_INDIVIDUAL_FACTOR_STD);
            this.dataVo.setStd_type(EnumFactorStdType.SELECTION.toString());
        }
    }

    private NVO_BASELAND_INDIVIDUAL_FACTOR_STD getDefVo(int n) {
        NVO_BASELAND_INDIVIDUAL_FACTOR_STD nVO_BASELAND_INDIVIDUAL_FACTOR_STD = new NVO_BASELAND_INDIVIDUAL_FACTOR_STD();
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setCity(this.dataVo.getCity());
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setDist(this.dataVo.getDist());
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setYear(this.dataVo.getYear());
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setVersion(this.dataVo.getVersion());
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setBaseno(this.dataVo.getBaseno());
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setItem(this.dataVo.getItem());
        nVO_BASELAND_INDIVIDUAL_FACTOR_STD.setLevel(n);
        return nVO_BASELAND_INDIVIDUAL_FACTOR_STD;
    }
    ...
    extends AnchorPane {
        ...
        NVO_BASELAND_INDIVIDUAL_FACTOR_STD stdVo = null;
        AnchorPane panel = null;
        String level = null;

        public void updateVo() {
            this.stdVo = new NVO_BASELAND_INDIVIDUAL_FACTOR_STD();
            this.stdVo.setCity(SystemIndividualFactor_editController.this.dataVo.getCity());
            this.stdVo.setDist(SystemIndividualFactor_editController.this.dataVo.getDist());
            this.stdVo.setYear(SystemIndividualFactor_editController.this.dataVo.getYear());
            this.stdVo.setVersion(SystemIndividualFactor_editController.this.dataVo.getVersion());
            this.stdVo.setBaseno(SystemIndividualFactor_editController.this.dataVo.getBaseno());
            this.stdVo.setItem(SystemIndividualFactor_editController.this.dataVo.getItem());
        ...
        public LevelPanel(AnchorPane anchorPane, NVO_BASELAND_INDIVIDUAL_FACTOR_STD nVO_BASELAND_INDIVIDUAL_FACTOR_STD, String string, String string2) {
            this.panel = anchorPane;
            this.level = string2;
            this.stdVo = nVO_BASELAND_INDIVIDUAL_FACTOR_STD;
            for (Node node : anchorPane.getChildren()) {
            ...
            this.a_symbol.setItems(FXCollections.observableArrayList((Collection)SystemIndividualFactor_editController.this.getCompatorList()));
            this.b_symbol.setItems(FXCollections.observableArrayList((Collection)SystemIndividualFactor_editController.this.getCompatorList()));
            this.c_symbol.setItems(FXCollections.observableArrayList((Collection)SystemIndividualFactor_editController.this.getCompatorList()));
            this.d_symbol.setItems(FXCollections.observableArrayList((Collection)SystemIndividualFactor_editController.this.getCompatorList()));
    ...
    extends AnchorPane {
        ...
        public CustomPanel(AnchorPane anchorPane, NVO_BASELAND_INDIVIDUAL_FACTOR_STD nVO_BASELAND_INDIVIDUAL_FACTOR_STD, String string, String string2) {
            this.panel = anchorPane;
            this.level = string2;
            for (Node node : anchorPane.getChildren()) {
                String string3 = node.getId();
            ...
            this.btDef.setOnAction(actionEvent -> this.contents.setText(SystemIndividualFactor_editController.this.defaultText));
            this.lbLevelName.setText(string);
            if (nVO_BASELAND_INDIVIDUAL_FACTOR_STD != null && !StringProcess.isEmpty(nVO_BASELAND_INDIVIDUAL_FACTOR_STD.getContents()) && EnumFactorStdType.CUSTOM.toString().equals(SystemIndividualFactor_editController.this.dataVo.getStd_type())) {
                this.contents.setText(nVO_BASELAND_INDIVIDUAL_FACTOR_STD.getContents());
            }
        }
